...
from .gui_common import (
    ...
    _fmt_number,
    _show_table_copy_menu,
)
from .ozon_gui import OzonDialog, OzonPanel
...
class SettingsDialog(QDialog):
    ...
    def __init__(
        ...
        last_check = current.last_update_check.strip()
        self.last_update_check_label = QLabel(
            f"Последняя проверка: {last_check}" if last_check
            else "Последняя проверка: ни разу не проходила",
            self,
        )
        self.last_update_check_label.setObjectName("subtitleLabel")
        self.last_update_check_label.setToolTip(
            "Когда Dazzle последний раз смог прочитать манифест обновлений.\n"
            "Если проверка не проходила, причина будет в логе."
        )
        updates_layout.addWidget(self.last_update_check_label, 2, 0, 1, 2)

        self.open_log_dir_btn = QPushButton("Открыть папку с логом", self)
        self.open_log_dir_btn.setToolTip(
            "Открывает папку с файлом dazzle.log — его можно приложить к сообщению\n"
            "о проблеме, там записаны детали ошибок."
        )
        self.open_log_dir_btn.clicked.connect(self._open_log_dir)
        updates_layout.addWidget(self.open_log_dir_btn, 3, 0, 1, 2)
    ...
    def _open_log_dir(self) -> None:
        """Показать продавцу папку с логом — чтобы приложить файл к сообщению."""
        directory = log_dir()
        try:
            os.startfile(str(directory))  # noqa: S606 — открываем папку проводником
        except Exception:
            # Проводник может не открыться (например, в урезанной сборке Windows):
            # тогда просто показываем путь, скопировать его можно мышкой.
            QMessageBox.information(self, "Лог", f"Файл лога лежит здесь:\n{directory}")
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.suppliers: list[tuple[int, str]] = []
        self.users: list[tuple[int, str]] = []
        self.shops: list[tuple[int, str]] = []
    ...
    def _build_ui(self) -> None:
        ...
        self.ozon_btn = QPushButton("Ozon CSV", self)
        self.ozon_btn.setObjectName("subtleBtn")
    ...
    def _check_for_updates(
        ...
        *,
        ...
        try:
            ...
            # В файловый лог пишем всегда. При фоновой проверке это единственный
            # след того, что автообновление до магазина не доезжает: раньше
            # запись зависела от режима LOG, который у продавца выключен, и
            # сбой не оставлял следов вообще.
            get_logger().warning("Проверка обновлений не удалась (%s): %s", manifest_url, exc)
            if interactive:
                QMessageBox.warning(self, "Обновления", f"Не удалось проверить обновления:\n{exc}")
            elif self.debug_log_enabled:
                self._log(f"Проверка обновлений: ошибка: {exc}")
            return

        self.app_settings.last_update_check = datetime.now().strftime("%d.%m.%Y %H:%M")
        self._save_settings(silent=True)
    ...
    def _open_ozon_dialog(self) -> None:
        ...
        if self._is_ui_busy():
            return
        if self.db is None or self.matcher is None:
            QMessageBox.warning(self, "Ozon", "Сначала подключите базу Tirika.")
            return
        if not self.suppliers or not self.users or not self.shops:
            try:
                self.suppliers = self.db.list_suppliers()
                self.users = self.db.list_users()
                self.shops = self.db.list_shops()
            except Exception as exc:
                self._error("Не удалось загрузить справочники для Ozon", exc=exc)
                return
    ...
    def _on_invoice_loaded(self, payload: object) -> None:
        try:
            ...
            self._report_parse_warnings(invoice)
    ...
    def _report_parse_warnings(self, invoice: ParsedInvoice) -> None:
        """Сказать о претензиях к файлу сразу, а не когда продавец нажмёт «Импорт»."""
        warnings = [text.strip() for text in invoice.parse_warnings if text.strip()]
        if not warnings:
            return
        for text in warnings:
            self._log(f"Накладная: {text}")
        QMessageBox.warning(self, "Накладная загружена с оговорками", "\n\n".join(warnings))
    ...
    def _error(self, text: str, exc: Exception | None = None) -> None:
        ...
        # Раньше на экран уходила одна строка, а трассировка оставалась только в
        # файле — разбирать приходилось по скриншотам от продавца. Теперь детали
        # можно развернуть и одной кнопкой скопировать в сообщение.
        ...
        if details:
            ...
            copy_btn = box.addButton("Скопировать детали", QMessageBox.ActionRole)
        qt_exec(box)
        if copy_btn is not None and box.clickedButton() is copy_btn:
            report = "\n".join(
                [
                    f"{display_app_name()} {APP_VERSION}",
